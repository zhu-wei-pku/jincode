...
c     Subroutine to find te given t1
      ...
      real*8 t2, u0, t1, rhos, piex, piey, theta, db, dep, b, q,ab,aep !input
      real*8 te, t0 !output
...
c     Given u0, calculate the time of the first caustic crossing
      ...
      real*8 t2, u0, te, rhos, piex, piey, theta, db, dep, b_0, q,ab,aep !input
      real*8 b, ep, ep_0, tbinary, dcm, dt
      real*8 t1 !Output
      real*8 t0 !internal
...
c      write(6,1000) t2,u0,te,rhos,piex,piey,theta,db,dep,b_0,q,ab,aep
c 1000 format(13f8.4)
...
c     Check that the boundaries are good.
 100  continue
         ...
         if (t1_lo+t1par.lt.4650.)then
            dt = (4650.-tbinary)/365.25
         else if (t1_lo+t1par.gt.4850.)then
            dt = (4850.-tbinary)/365.25
         else
            dt = (t1_lo+t1par-tbinary)/365.25
         endif

         b = b_0 + db*dt + 0.5*ab*dt**2
         ep = ep_0 + dep*dt + 0.5*aep*dt**2
...
c     Shift the interval so that t1_lo is correct
 ...
 200     continue
         ...
         if (t1_hi+t1par.lt.4650.)then
            dt = (4650.-tbinary)/365.25
         else if (t1_hi+t1par.gt.4850.)then
            dt = (4850.-tbinary)/365.25
         else
            dt =(t1_hi+t1par-tbinary)/365.25
         endif
...
c     Use a binary search to find t1 given t0.
      do i = 1, n
         ...
         if (t1_try+t1par.lt.4650.)then
            dt = (4650.-tbinary)/365.25
         else if (t1_try+t1par.gt.4850.)then
            dt = (4850.-tbinary)/365.25
         else
            dt = (t1_try+t1par-tbinary)/365.25
         endif
c         write(6,*) 't1_try', t1_try, t1_lo, t1_hi
c         write(6,*) dt, db, dep, ab, aep
...
c     Output: t2
      ...
      real*8 t0, u0, te, rhos, piex, piey, theta, db, dep, b_0, q,ab,aep !input
      real*8 b, ep, ep_0, tbinary, dcm,dt
      real*8 t2 !Output
...
c$$$      write(6,1000) t0,u0,te,rhos,piex,piey,theta,db,dep,b_0,q,ab,aep
c$$$ 1000 format(13f8.4)
...
c     Check that the boundaries are good.
 100  continue
         ...
         if (t2_lo+t2par.lt.4550.)then
            dt = (4550.-tbinary)/365.25
         else if (t2_lo+t2par.gt.4850.)then
            dt = (4850.-tbinary)/365.25
         else
            dt = (t2_lo+t2par-tbinary)/365.25
         endif
 ...
 200     continue
         ...
         if (t2_hi+t2par.lt.4550.)then
            dt = (4550.-tbinary)/365.25
         else if (t2_hi+t2par.gt.4850.)then
            dt = (4850.-tbinary)/365.25
         else
            dt = (t2_hi+t2par-tbinary)/365.25
         endif
...
c     Use a binary search to find t2 given t0.
      do i = 1, n
         ...
         if (t2_try+t2par.lt.4550.)then
            dt = (4550.-tbinary)/365.25
         else if (t2_try+t2par.gt.4850.)then
            dt = (4850.-tbinary)/365.25
         else
            dt= (t2_try+t2par-tbinary)/365.25
         endif
c         write(6,*) 't2_try', t2_try, t2_lo, t2_hi
c         write(6,*) dt, db, dep, ab, aep, b_0, q
